...
private:
    ...
    void addRoundKey(const vector<uint8_t>& roundKey) {
        for (int i = 0; i < 16; i++)
            state[i] ^= roundKey[i];
    }
...
public:
    AES() {}
    AES(vector<uint8_t> inp) : state(inp) {}

    vector<uint8_t> encrypt(const vector<uint8_t>plain ,const vector<uint8_t>& key) {
        state = plain;
    ...
    vector<uint8_t> decrypt(const vector<uint8_t>ciper ,const vector<uint8_t>& key) {
        state = ciper;
        auto roundKeys = keyExpansion(key);
